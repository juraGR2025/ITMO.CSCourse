...
namespace RegistrationForm
{
    public partial class Form1 : Form
    {
        ...
        private void checkBox1_CheckedChanged(object sender, EventArgs e)
        {
           ...
           Для защиты текстового поля, появляющегося при установке галочки в чекбоксе
 "Расширенные возможности", вручную определяем событие KeyPress.*/
            }
            else
            {
                //Дополнительные элементы удаляются если флажок с checkBox1 снят.
                int lcv;
        ...
        {
            errorProvider1.SetError(textBox1, "Must be letter");//Использование errorProvider

            if (char.IsDigit(e.KeyChar))//Что значит запись char.IsDigit(e.KeyChar)?
            {
                e.Handled = true;
